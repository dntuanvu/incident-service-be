...
export class IncidentController {
  ...
  async raise(
    ...
    const generatedId = await this.incidentService.insertIncident(
      type,
      detail,
      creator,
    );
  ...
  @UseGuards(JwtAuthGuard)
  @Get('/assignee/:assignee_id')
  async getByAssignee(
    @Request() req,
    @Param('assignee_id') assigneeId: string,
  ) {
    const incidents = await this.incidentService.getIncidentByAssignee(
      assigneeId,
    );
    return {
      incidents,
