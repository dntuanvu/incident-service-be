...
export class IncidentService {
  ...
  async getSingleIncident(incidentId: string) {
    ...
    return {
      ...
      assignee:
        assignee === null ? null : { id: assignee.id, email: assignee.email },
  ...
  async getIncidentByAssignee(assigneeId: string) {
    const incidents = await this.incidentModel
      .find({ assignee: assigneeId })
      .exec();

    console.log(
      'getIncidentByAssignee, incidents=' + JSON.stringify(incidents),
    );
    return incidents.map((inc) => ({
      id: inc.id,
      type: inc.type,
